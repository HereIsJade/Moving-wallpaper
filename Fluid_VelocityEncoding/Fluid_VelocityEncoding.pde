...
//      pg_tex_handle[0] = pg.getTexture().glName
  ...
  int argb;
  float mx = viewport_w-400, mx_prev = 0, vx= 0;
  float my = -20, my_prev = 0, vy= 0;
  ...
  public void drawVelocity(PGraphics pg, int texture_type){
    ...
    if(vM > 0){
      ...
      if(texture_type == 1){
        ...
        //  pg.line(0,i*spaceY,width/2,i*spaceY);
        //}

        //for(int i=0;i<n;i++){
        //  pg.line(i*spaceX,0,width,height-i*spaceY);
        //  pg.line(width,i*spaceY,i*spaceX,height);
        //}
      ...
      // pg.ellipse(0, 0, 50, 50 - vM);
      pg.ellipse(0, 0, 50, 40);
